...
from dqn_zoo.c51 import dceo_agent as agent

# Relevant flag values are expressed in terms of environment frames.
FLAGS = flags.FLAGS
flags.DEFINE_string('environment_name', 'montezuma_revenge', '')
...
flags.DEFINE_float('exploration_epsilon_decay_frame_fraction', 0.005, '')
flags.DEFINE_float('eval_exploration_epsilon', 0.001, '')
flags.DEFINE_integer('target_network_update_period', int(32e3), '')
...
def main(argv):
  ...
  rep_optimizer = optax.adam(
      learning_rate=FLAGS.learning_rate, eps=FLAGS.optimizer_epsilon)
  if FLAGS.max_global_grad_norm > 0:
    rep_optimizer = optax.chain(
        optax.clip_by_global_norm(FLAGS.max_global_grad_norm), rep_optimizer)
  ...
  checkpoint = parts.AgentParamsCheckpoint(directory=FLAGS.results_csv_path)
