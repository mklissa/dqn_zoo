...
from dqn_zoo.dqn.pvn_functions import create_train_state, get_pvn
from dqn_zoo.dqn.indicator_functions import create_indicator_state
from dqn_zoo.dqn.pvn_train import pvn_train_step
...
class Agent(parts.Agent):
  ...
  def __init__(
    ...
    self._laplacian_params = lap_network.init(network_rng_key,
                                       sample_network_input[None, ...])
    self._lap_opt_state = optimizer.init(self._laplacian_params)
    ...
    self._rng_key, indicator_key, pvn_key = jax.random.split(self._rng_key, 3)
    self.indicator_state = create_indicator_state(
      sample_network_input[None, ...], indicator_key, learning_rate, lap_dim)
    self.pvn_state = create_train_state(
      sample_network_input[None, ...], pvn_key, learning_rate, lap_dim)
    self.pvn_train_step = jax.jit(pvn_train_step)
    self.get_pvn = jax.jit(get_pvn)
  ...
  def step(self, timestep: dm_env.TimeStep) -> parts.Action:
    ...
    if self._option_term:
      ...
      if np.random.rand() < self.exploration_epsilon:
        ...
        if np.random.rand() < self._option_prob:
  ...
  def _learn(self) -> None:
    ...
    if  (
      ...
      for o, option in enumerate(self.options):
        transitions = self._replay.sample(self._batch_size)
        rep =  self.get_pvn(self.pvn_state,
          self.pvn_state.params, transitions.s_t).predictions
        rep = np.array(rep)
        intr_rew = rep[:, o]
  ...
  def get_rep(self, cover):
    rep =  self.pvn_state.apply_fn(self.pvn_state.params, cover).predictions
    rep = np.array(rep)
    # output = np.linalg.norm(rep[0] - rep, axis=1)
    # reward =  self.indicator_state.apply_fn(
    #     self.indicator_state.params, cover).rewards
