...
class PVNOutputs(typing.NamedTuple):
  phi: jnp.ndarray
...
def minigrid_network(num_outputs: int) -> NetworkFn:
  """Minigri DCEO network, expects `uint8` input."""

  def net_fn(inputs):
    """Function representing DCEO network."""
    network = hk.Sequential([
...
def rubiks_network(num_outputs: int, actor: bool = False) -> NetworkFn:
  """Minigrid DCEO network, expects `uint8` input."""
  ...
  if actor:
    def net_fn(inputs):
      ...
      network = hk.Sequential([
          lambda x: x.astype(jnp.float32),
          hk.Linear(
            num_outputs, with_bias=False, w_init=hk.initializers.Constant(0.)),
      ])
      return QNetworkOutputs(q_values=network(inputs))
  else:
    def net_fn(inputs):
...
class NatureDQNNetwork(nn.Module):
  ...
  output_dim: int
  normalization: float = 255.

  @nn.compact
  def __call__(self, x):
    initializer = nn.initializers.xavier_uniform()
    x = x.astype(jnp.float32) / self.normalization
    ...
    x = nn.relu(x)
    output = nn.Dense(features=self.output_dim, use_bias=False,
                        kernel_init=initializer)(x)
    return PVNOutputs(phi=x, predictions=output)


class PartiallyNatureDQNNetwork(nn.Module):
  """The convolutional network used to compute the agent's Q-values."""
  output_dim: int
  normalization: float = 255.
