...
class Transition(typing.NamedTuple):
  ...
  skill_tm1: Optional[Any] = -1
  index_tm1: Optional[Any] = -1
  index_t: Optional[Any] = -1
...
class TransitionAccumulator:
  ...
  def step(self, timestep_t: dm_env.TimeStep,
           a_t: parts.Action, skill_t: Any = -1) -> Iterable[Transition]:
    ...
    if self._timestep_tm1 is None:
      ...
      self._a_tm1 = a_t
      self._skill_tm1 = skill_t
      return  # Empty iterable.
    else:
      if isinstance(self._timestep_tm1.observation, dict):
        transition = Transition(
            s_tm1=self._timestep_tm1.observation['obs'],
            a_tm1=self._a_tm1,
            r_t=timestep_t.reward,
            discount_t=timestep_t.discount,
            s_t=timestep_t.observation['obs'],
            skill_tm1=self._skill_tm1,
            index_tm1=self._timestep_tm1.observation['state'],
            index_t=timestep_t.observation['state'],
        )
      else:
        transition = Transition(
...
def _build_n_step_transition(transitions):
  ...
  return Transition(
      ...
      skill_tm1=transitions[0].skill_tm1,
      index_tm1=transitions[0].index_tm1,
      index_t=transitions[0].index_t,
  )
...
class RMS(object):
  ...
  def __call__(self, x):
    ...
    new_S = (self.S * self.n + np.var(x, axis=0) * bs +
              np.square(delta) * self.n * bs /
              (self.n + bs)) / (self.n + bs)
    self.M = new_M
    self.S = new_S
    self.n += bs
